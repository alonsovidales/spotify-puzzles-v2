...
class CatVsDog:
    ...
    def resolve(self):
        ...
        byDogLoveTotalVotes = {}
        for dogInLove, dogByLoveVotes in byDogLove.items():
            byDogLoveTotalVotes[dogInLove] = 0
            for vote in dogByLoveVotes:
                byDogLoveTotalVotes[dogInLove] += votes[vote]

        byDogHateTotalVotes = {}
        for dogInHate, dogByHateVotes in byDogHate.items():
            byDogHateTotalVotes[dogInHate] = 0
            for vote in dogByHateVotes:
                byDogHateTotalVotes[dogInHate] += votes[vote]

        byCatLoveTotalVotes = {}
        for catInLove, catByLoveVotes in byCatLove.items():
            byCatLoveTotalVotes[catInLove] = 0
            for vote in catByLoveVotes:
                byCatLoveTotalVotes[catInLove] += votes[vote]

        byCatHateTotalVotes = {}
        for catInHate, catByHateVotes in byCatHate.items():
            byCatHateTotalVotes[catInHate] = 0
            for vote in catByHateVotes:
                byCatHateTotalVotes[catInHate] += votes[vote]

        if self.__debug:
            print "byDogLoveTotalVotes: %s" % (byDogLoveTotalVotes)
            print "byDogHateTotalVotes: %s" % (byDogHateTotalVotes)
            print "byCatLoveTotalVotes: %s" % (byCatLoveTotalVotes)
            print "byCatHateTotalVotes: %s" % (byCatHateTotalVotes)
        ...
        for catLover in catLovers:
            ...
            votes[catLover[0]] += totalDogLovers - byDogLoveTotalVotes.get(catLover[1][1], 0) - byDogHateTotalVotes.get(catLover[1][0], 0)
